...
def fetch_price_data(symbols, start, end):
    ...
    for name, symbol in symbols.items():
        ...
        if not df.empty and "Close" in df.columns:
            ...
            st.warning(f"⚠️ No data for {name} ({symbol})")
...
valid_sectors = [col for col in data.columns if col != "NIFTY_50"]
if not valid_sectors:
    st.error("❌ No valid sector data available for comparison.")
    st.stop()

rs_df = data[valid_sectors].div(data["NIFTY_50"], axis=0)
